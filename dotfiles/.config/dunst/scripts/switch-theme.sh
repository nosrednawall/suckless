...
##     -u|--update <name>   Optional. Update existing theme from X resources.
##
## Description
##
...
create_theme() {
    ...
    if [ -f "$theme_file" ]; then
        echo "Theme '${theme_name}' already exists. Use -u to update from X resources."
        return 1
    fi
...
# Update theme from X resources
update_theme_from_xresources() {
    local theme_name="$1"
    local theme_file="$themes_dir/${theme_name}.conf"

    if [ ! -f "$theme_file" ]; then
        echo "Theme '${theme_name}' not found. Use -c to create it first."
        list_themes
        return 1
    fi

    echo "Updating theme '${theme_name}' from X resources..."

    # Create backup of current theme
    backup_theme_file="$backup_dir/${theme_name}.backup.$(date +%Y%m%d_%H%M%S).conf"
    cp "$theme_file" "$backup_theme_file"
    echo "Theme backup created: $backup_theme_file"
    ...
    # Update theme file
    cat > "$theme_file" << EOF
# Theme: ${theme_name} (updated from X resources on $(date))
# Previous backup: $backup_theme_file
...
EOF
    ...
    echo "Theme '${theme_name}' updated from X resources."
    echo ""
    echo "If the theme was currently applied, you may want to reapply it:"
    echo "  $script_name $theme_name"
}
...
if [ -n "$UPDATE_THEME" ]; then
    update_theme_from_xresources "$UPDATE_THEME"
    exit 0
fi
...
echo "  $script_name -u mytheme            # Update existing theme from X resources"
